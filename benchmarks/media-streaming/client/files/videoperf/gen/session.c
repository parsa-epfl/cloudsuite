...
create_conn (Sess *sess, struct Conn_Info *ci)
{
	...
	//	strcpy(ci->conn->local_ip, spriv->local_ip);
	if (sess->port != 0)
	{
	...
	strcpy(ci->conn->local_ip, sess->local_ip);
	ci->conn->log_index = sess->log_index;
