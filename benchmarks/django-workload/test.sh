...
sudo docker network create --opt com.docker.network.bridge.name=django --attachable \
...
sudo docker run -tid -h memcached --name memcached_container --network              \
       django_network -p 11211:11211 --ip 10.10.10.10 memcached-webtier
...
sudo docker run -tid --privileged -h cassandra -e SYSTEM_MEMORY=8 --name cassandra_container           \
           --network django_network -p 9042:9042 --ip 10.10.10.11 cassandra-webtier
...
sudo docker run -tid -h graphite --name graphite_container --network django_network \
           -p 80:80 --ip 10.10.10.12 graphite-webtier
...
sudo docker run -tid -h uwsgi --name uwsgi_container --network django_network        \
           -p 8000:8000 --ip 10.10.10.13 -e GRAPHITE_ENDPOINT=10.10.10.12            \
