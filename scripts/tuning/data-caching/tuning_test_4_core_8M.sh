...
# Run the test
docker run --cpuset-cpus="32-35" --name dc-server --net caching_network -d cloudsuite/data-caching:server_ARM -t 4 -m 8192 -n 550
docker run --cpuset-cpus="48-55" -d --name dc-client --net caching_network cloudsuite/data-caching:client bash -c 'cd /usr/src/memcached/memcached_client/; \
	...
	for i in `seq 155000 500 260000`; do \
		echo rps: $i; \
