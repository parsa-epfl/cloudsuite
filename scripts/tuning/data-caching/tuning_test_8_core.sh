...
docker stop dc-server-1 || true
docker stop dc-server-2 || true
docker stop dc-client || true
docker rm dc-server-1 || true
docker rm dc-server-2 || true
docker rm dc-client || true

# Run the test
docker run --cpuset-cpus="32-35" --name dc-server-1 --net caching_network -d cloudsuite/data-caching:server_ARM -t 4 -m 2048 -n 550
docker run --cpuset-cpus="36-39" --name dc-server-2 --net caching_network -d cloudsuite/data-caching:server_ARM -t 4 -m 2048 -n 550
docker run --cpuset-cpus="48-55" -d --name dc-client --net caching_network cloudsuite/data-caching:client bash -c 'cd /usr/src/memcached/memcached_client/; \
	echo dc-server-1, 11211 > docker_servers.txt; \
	echo dc-server-2, 11211 > docker_servers.txt; \
	...
	for i in `seq 10000 500 300000 | shuf`; do \
		echo rps: $i; \
		./loader -a ../twitter_dataset/twitter_dataset_30x -s docker_servers.txt -g 0.8 -T 5 -c 200 -w 16 -e -r $i & \
		sleep 355; \
		for j in  {1..2}j; do \
			mpstat -P ALL 25 1; \
		done; \
