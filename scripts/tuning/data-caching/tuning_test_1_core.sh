...
# Run the test
docker run --cpuset-cpus="32-32" --name dc-server --net caching_network -d cloudsuite/data-caching:server_ARM -t 1 -m 2048 -n 550
docker run --cpuset-cpus="48-55" -d --name dc-client --net caching_network cloudsuite/data-caching:client bash -c 'cd /usr/src/memcached/memcached_client/; \
	...
	for i in `seq 1000 500 80000 | shuf`; do \
		echo rps: $i; \
