...
- `--min=<# of seconds>`: the minimum number of milliseconds between two consecutive requests of a specific user. The default value is 1000.
- `--max=<# of seconds>`: the maximum number of milliseconds between two consecutive requests of a specific user. The default value is 1500.
- `--type=<THINKTIME | CYCLETIME>`: This option has two possible values: `THINKTIME` and `CYCLETIME`. For the former value, the latency before sending a new request (determined by `--min` and `--max`) is measured compared to when the last request finishes. On the other hand, the `CYCLETIME` policy calculates the latency from the moment a request is sent to the server. The default value is `THINKTIME`.
